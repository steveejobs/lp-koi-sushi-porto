...
export function TableVisualSection() {
  const baseImage = cinematicGallery.find(
    (item) => item.role === "feature-base",
  );
  const video = cinematicGallery.find(
    (item) => item.role === "feature-video",
  );

  if (!baseImage || !video) return null;
        ...
        >
          <div>
            ...
            <h2 className="mt-5 max-w-2xl text-4xl font-black leading-[1.02] md:text-6xl">
              Sushi e cozinha chinesa servidos no Koi.
            </h2>
          </div>
          <p className="max-w-xl text-base font-semibold leading-7 text-[#efe2c8]/78 md:justify-self-end md:text-lg">
            Peças frescas, pratos quentes e detalhes preparados para comer no
            restaurante ou pedir em Take Away.
          </p>
        </Reveal>
